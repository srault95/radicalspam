...
SERVER=""
COUNTER_FILE=${RS_BASE}/addons/nagios/var/stats_postfix_queue_counters.txt
VCASE=$1
VWARN=$2
VCRIT=$3
...
if [ $# -gt 3 ]; then
   ...
   if [ -e ${RS_BASE}/addons/nagios/var/$SERVER-stats_postfix_queue_counters.txt ]; then
      COUNTER_FILE=${RS_BASE}/addons/nagios/var/$SERVER-stats_postfix_queue_counters.txt
   else
      exit $STATE_UNKNOWN
   fi
